# platform-checks Specification

## Purpose

定义 weops-inspect 平台组件层（ES / Redis 单点 / Redis Sentinel / MongoDB / RabbitMQ /
bkmonitorv3 依赖 / Service 段）的检查规则与告警判定单一事实源。本 capability 把
"是否告警"的阈值比较职责集中在 checker 层，统一使用四元状态 `OK/Warn/Unknown/Notice`
作为渲染、Summary 计数与告警邮件的统一信号。
