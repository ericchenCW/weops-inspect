# bkmonitorv3-collection Specification

## Purpose

定义对蓝鲸监控后台 V3 (bkmonitorv3) 模块的进程级巡检与模块依赖连通性采集规则; 进程级部分复用现有 service.go 流水线, 依赖连通性部分对齐 `bk-install/health_check/deploy_check.py:bkmonitorv3()`。
